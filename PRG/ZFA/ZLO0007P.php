      ...
      if (isset($_SESSION['clickPais']) && isset($_SESSION['clickCia'])) {
        if ($_SESSION['clickCia']==1) {
          ...
          || $ciaFiltro==74 || $ciaFiltro==75 || $ciaFiltro==76 || $ciaFiltro==78 || $ciaFiltro==82 || $ciaFiltro==85) {
            $paisFiltro=1;
          }elseif ($ciaFiltro==20 || $ciaFiltro==22) {
            $paisFiltro=2;
          }elseif ($ciaFiltro==49 || $ciaFiltro==66 || $ciaFiltro==69 || $ciaFiltro==71 || $ciaFiltro==86) {
            $paisFiltro=3;
          }elseif ($ciaFiltro==48 || $ciaFiltro==53 || $ciaFiltro==61 || $ciaFiltro==62) {
            $paisFiltro=4;
          }elseif ($ciaFiltro==60 || $ciaFiltro==80) {
            $paisFiltro=5;
        ...
        if ($_SESSION['clickPais']==1) {
          ...
          if ($paisFiltro==1) {
            ...
            $cia=" AND CODCIA IN(20,22)";
          }elseif ($paisFiltro==3) {
            $cia=" AND CODCIA IN(49,66,69,71,86)";
          }elseif ($paisFiltro==4) {
            $cia=" AND CODCIA IN(48,53,61,62)";
          }elseif ($paisFiltro==5) {
            $cia=" AND CODCIA IN(60,80)";
          }elseif ($paisFiltro==6) {
          ...
          <div class="position-relative">
              <form id="formFiltros" action="../../assets/php/ZFA/ZLO0007P/filtroLogica.php" method="POST">
                ...
                <div class="col-sm-12 col-lg-4 mt-2">
                        ...
                        <option value="999">Todas las compañias</option>
                          <?php
             ...
             <div id="carouselExampleDark" class="carousel carousel-dark slide"  data-bs-ride="carousel">
                ...
                <div class="carousel-item active">
                                  <div id="grafica1" class="me-5 ms-5">
                                ...
                                <div id="grafica1" class="me-5 ms-5">
                                      ...
                                      <h5 class="text-center">Venta y Venta 2 Años Antes </h5><br>
                                      <h5 class="text-center" id="lblRadial"></h5>
                                          <div class="d-flex justify-content-evenly">
                                          <div>
                                          <label class="responsive-font-example fw-bold">Venta</label>
             ...
             <hr>
                    <div class="table-responsive mt-3" style="width:100%">
                          <table id="myTableVendedorVentas" class="table stripe table-hover " style="width:100%">
                            <thead>
                                <tr>
                                    <th class="responsive-font-example text-start">Vendedor</th>
                                    <th  class="responsive-font-example text-end" >Transacciones</th>
                                    <th  class="responsive-font-example text-end" >Unidades</th>
                                    <th  class="responsive-font-example text-end" >Venta</th>
                                    <th  class="responsive-font-example text-end">Promedio de Ventas</th>
                                    <th  class="responsive-font-example text-end">Venta Año Anterior</th>
                                    <th  class="responsive-font-example text-end">Variación</th>
                                    <th  class="responsive-font-example text-end">Crecimiento</th>
                                    <th  class="responsive-font-example text-end">Venta 2 Años Antes</th>
                                    <th  class="responsive-font-example text-end">Variación2</th>
                                    <th  class="responsive-font-example text-end">Crecimiento</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                 ...
                                 $venano1=0; $venano2=0;
                                  while($rowVen = odbc_fetch_array($resultVentas)){
                                    ...
                                    print '<tr>';
                                    print '<td  class="responsive-font-example text-start fw-bold">'.rtrim(utf8_encode($rowVen['MAENO3'])).'</td>';
                                    print '<td  class="responsive-font-example text-end">'.number_format($rowVen['CANTRA'],0).'</td>';
                                    print '<td  class="responsive-font-example text-end">'.$cantidad.'</td>';
                                    print '<td  class="responsive-font-example text-end fw-bold">'.(($rowVen['VALOR1']!=0)? number_format($rowVen['VALOR1'],2):' ').'</td>';
                                    print '<td  class="responsive-font-example text-end fw-bold">'.number_format($promVen,2).'</td>';
                                    print '<td  class="responsive-font-example text-end fw-bold">'.(($rowVen['VALOR2']!=0)? number_format($rowVen['VALOR2'],2):' ').'</td>';
                                    print ($variacion1<0)? '<td  class="responsive-font-example text-end text-danger">'.(($variacion1!=0)? number_format($variacion1,2):' ').'</td>':'<td  class="responsive-font-example text-end text-success">'.(($variacion1!=0)? number_format($variacion1,2):' ').'</td>';
                                    print ($crecimiento1<0)? '<td  class="responsive-font-example text-end text-danger">'.number_format($crecimiento1,0).'%'.'</td>': ($crecimiento1>0)? '<td  class="responsive-font-example text-end text-success">'.number_format($crecimiento1,0).'%'.'</td>':'<td  class="responsive-font-example text-end">'.number_format($crecimiento1,0).'%'.'</td>';
                                    print '<td  class="responsive-font-example text-end fw-bold">'.(($rowVen['VALOR3']!=0)? number_format($rowVen['VALOR3'],2):' ').'</td>';
                                    print ($variacion2<0)? '<td  class="responsive-font-example text-end text-danger">'.(($variacion2!=0)? number_format($variacion2,2):' ').'</td>':'<td  class="responsive-font-example text-end text-success">'.(($variacion2!=0)? number_format($variacion2,2):' ').'</td>';
                                    print ($crecimiento2<0)? '<td  class="responsive-font-example text-end text-danger">'.number_format($crecimiento2,0).'%'.'</td>': ($crecimiento2>0)? '<td  class="responsive-font-example text-end text-success">'.number_format($crecimiento2,0).'%'.'</td>':'<td  class="responsive-font-example text-end">'.number_format($crecimiento2,0).'%'.'</td>';
                                    print '</tr>';
