<?php
 ...
 //VALIDACIONES JS
   ...
   switch ($_SESSION['filtro']) {
    case '1': //UNIDADES
      $_SESSION['logicSql'] ="SELECT COD.CODSEC CODSEC,MES.CODCIA ID,DES.NOMCIA COMDES, ' ' AS MON, 
      ...
      ORDER BY T2.CODSEC";
        ...
        $promedios="SELECT TRA.CODSEC,TRA.ID ID,TRA.COMDES COMDES,' ' AS MON, (UNIDM.SUBDIA/TRA.DIATRA) PRODIA,
      ...
      INNER JOIN LBPRDDAT/LO0686 AS T2 ON T2.CODCIA = T1.CODCIA
      INNER JOIN LBPRDDAT/LO0705 AS T3 ON T3.CODCIA = T1.CODCIA      
      ORDER BY T2.CODSEC)AS UNIANO ON UNIANO.ID=TRA.ID ORDER BY TRA.CODSEC";
      $resultpromedios=odbc_exec($connIBM,$promedios); 

      break;
      case '2'://TRANSACCIONES
        
        $_SESSION['logicSql'] ="SELECT CODSEC,ID,COMDES,' ' AS MON, COALESCE(TRA.NUMTRA, 0) SUBDIA, MESTRA SUBMES
        FROM (SELECT T7.CODSEC CODSEC, T1.CODCIA ID, T4.NOMCIA COMDES, T5.RELUS7 MON, MESTRA                                       
        ...
        LEFT OUTER JOIN lbprddat/LO0849 AS TRA 
        ON TF.ID = TRA.CODCIA AND TRA.FECPRO=".$_SESSION['FechaFiltro'] ." ORDER BY CODSEC";
      
        $comparacionAnual="SELECT CODSEC,ID,COMDES,' ' AS MON, ANOTRA ANO1, ANO2TRA ANO2, (ANOTRA - ANO2TRA) VARIA
