 ...
 $ano_actual=date("Y");
  ...
  if (!odbc_num_rows($resultFactor)) {
   ...
   while($rowFactor = odbc_fetch_array($resultFactor)){
    ...
    WHERE HISA60=".($anofiltro)." AND HISM37 BETWEEN 1 AND ".$mesfiltro." AND HIS056 IN (1,35,47,50,52,56,57,59,63,64,65,68,70,72,73,74,75,76,78,82,85)) T1
    ...
    SELECT HISM38, MAX(DESDES) as DESDES, SUM(HIS057) CANTIDAD, SUM(HISV22) VALOR
    FROM (SELECT HIS056, HISM38, HIS057, HISV22 FROM LBPRDDAT/hisa5804
    WHERE HISA60=".($anofiltro-1)." AND HISM37 BETWEEN 1 AND ".$mesfiltro." AND HIS056 IN (1,35,47,50,52,56,57,59,63,64,65,68,70,72,73,74,75,76,78,82,85)) T1
    INNER JOIN LBPRDDAT/DESARC T2 ON T1.HISM38=T2.DESCO1 AND T1.HIS056=T2.DESCOD
