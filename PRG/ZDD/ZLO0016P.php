...
<body>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            if (anoing == 0 & numemp == 0) {
                ...
                if (responseDepas.code == 200 && responseDepas.data.length > 1) {
                    ...
                    var secs1 = depas.split(",");
                    var depas1 = secs.split(",");
                    for (var i = 0; i < responseDepas.data.length; i++) {
                        var item = responseDepas.data[i];
            ...
            tiposChange();
            let id1 = 0; let nord1 = 0;
            if (tipoWeb != '') {
                $("#tiposDoc").val(tipoWeb).trigger('change');
                var camposId = paramsLength - 7;
                for (let i = 0; i < camposId; i++) {
                    var data = paramsData['CAM' + (i + 1) + ''].split(':');
                    if (data[1].trim() != 'CONSULTA' && data[1].trim() != 'CONSULTA2') {
                        if (data[0] == 'proveedor') {
                            var id = data[1].split("-");
                            var tipo = id[0];
                            var prov = id[1];
                            ...
                            if (data[0] == 'id' || data[0] == 'numero de orden') {
                                id1 = data[0] == 'id' ? data[1] : id1;
                                nord1 = data[0] == 'numero de orden' ? data[1] : nord1;
                        ...
                        if (data[1].trim() == 'CONSULTA2') {
                            cons2 = true;
                        }
                        if (id1 != 0 && nord1 != 0) {
                            var urlFac = "/API.LovablePHP/ZLO0016P/GetFactura/?CAM1=" + id1 + "&CAM2=" + nord1 + "";
                            var res = ajaxRequest(urlFac);
                            if (res.code == 200) {
                                $("#" + tipoWeb + i + "").val(res.data);
                            }
                        }
                    }
                }
            ...
            if (responseCampos.code == 200) {
                ...
                for (let i = 0; i < camposDes.length; i++) {
                    ...
                    if (camposDes[i].toLowerCase() == "proveedor") {
                        ...
                        htmlAppend +=
                            '<input class="inputsDoc fn" style="font-size:16px;"  type="text" data-placeholder-focus="false" required id="' +
                        ...
                        htmlAppend +=
                            '<input class="inputsDoc fn" style="font-size:14px;"  type="text"  data-placeholder-focus="false" required id="FechasDocs" onclick="showRange()"  oninput="noTextInput2(this)"/><button type="button" class="btn p-0 m-0 fs-5" onclick="vaciarInput2()"><i class="fa-solid fa-xmark fs-6"></i></button>';
                        htmlAppend +=
                            '<input class="d-none" id="originalRangeDocumento" /> <input class="d-none" id="valueTipo" />'
                    } else if (camposDes[i] == "Fecha de digitalización") {
                        htmlAppend +=
                            '<input class="inputsDoc fn" style="font-size:14px;"  type="text"  data-placeholder-focus="false" required id="FechasGrabs" onclick="showRange2()"  oninput="noTextInput4(this)"/><button type="button" class="btn p-0 m-0 fs-5" onclick="vaciarInput4()"><i class="fa-solid fa-xmark fs-6"></i></button>';
                        htmlAppend += '<input class="d-none" id="originalRangeGrabado" /> <input class="d-none" />'
                    } else {

                        htmlAppend +=
                            '<input class="inputsDoc fn"  type="text"  data-placeholder-focus="false" required id="' +
                            responseCampos.data[0]['TIPDOC'] + i + '"  />'
                    }
        ...
        function showCard(nomcard, usugra, fecgra, horgra, extdoc, urldoc, tipdoc, descrp, fecha, cam0, cam1, cam2, cam3, cam4, cam5, cam6, cam7, cam8, cam9, coddep, codsec, apr, modulo) {
            if (cons2) {
                findCons2(nomcard, usugra, fecgra, horgra, extdoc, urldoc, tipdoc, descrp, fecha, cam0, cam1, cam2, cam3, cam4, cam5, cam6, cam7, cam8, cam9, coddep, codsec, apr, modulo)
                $("#docInfo2").modal('show');
                cons2 = false;
            } else {
        ...
        function findCons2(nomcard, usugra, fecgra, horgra, extdoc, urldoc, tipdoc, descrp, fecha, cam0, cam1, cam2, cam3, cam4, cam5, cam6, cam7, cam8, cam9, coddep, codsec, apr, modulo) {
            ...
            //FACTURA
            $("#downloadFrame1").empty();
            let titleDoc = apr === 'A' ? `${cam5}<span class="badge text-bg-success text-white ms-4">Aprobado</span>` : cam5;
            $("#numfac").html(titleDoc);
            switch (extdoc) {
                case 'png':
                case 'jpg':
                case 'jpeg':
                    $("#downloadFrame1").append('<img src="' + urldoc +
                        '"  style="width:100%;" alt="">');
                    break;
                case 'pdf':
                    const box1 = document.getElementById('downloadFrame1');
                    let pdf = document.createElement('embed');
                    pdf.src = `http://172.16.15.20${urldoc}`;
                    pdf.style.width = '100%';
                    pdf.style.height = '85vh';
                    box1.appendChild(pdf);
                    break;
                default:
                    $("#downloadFrame1").append('<h5>El formato del documento no tiene previsualización.</h5>');
                    break;
            }
            ...
            $("#tipDoc1").text(textoSelect);
            $("#fechaDoc1").text(formatFecha(fecha));
            if (descrp == 'S-N') {
                descrp = '';
            }
            $("#descrpDoc1").text(descrp);
            ...
            $("#depaDoc1").text(coddep + " - " + codsec + " " + depa);
            $("#usuaGra1").text(usugra);
            $("#horaGra1").text(formatFecha(fecgra) + ' ' + formatTime(horgra));
            var campos = {
                cam0,
                cam1,
                cam2,
            ...
            const fetchPromises = []; let countOrdenes = 0; let divButtons = ''; let buttons = ''; let innerCarousel = ''; let carouselItems = '';
            fetch(urlFac, {
                method: 'POST',
                headers: {
                ...
                if (data.code == 200) {
                    ...
                    countOrdenes = response.length;
                    if (countOrdenes == 1) {
                    } else {
                        for (let k = 0; k < countOrdenes; k++) {
                            var baseUrl = "/API.LovablePHP/ZLO0016P/ListAsync/";
                            var queryParams = [];
                            queryParams.push("user=" + usuario);
                            if (tipdoc) queryParams.push("tipdoc=A");
                            if (cam0) queryParams.push("CAM1=" + cam0);
                            if (cam1) queryParams.push("CAM2=" + cam1);
                            if (cam4) queryParams.push("CAM4=" + response[k]);
                            var urlList = baseUrl + "?" + queryParams.join("&");

                            //armando el carrusel
                            fetchPromises.push(fetch(urlList, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json'
                                }
                                ...
                                if (data.code == 200) {
                                    ...
                                    buttons += `<button type="button" data-bs-target="#carouselExample" data-bs-slide-to="${k}" class="active" aria-current="true" aria-label="Slide ${k}"></button>`
                                    const response = data.data[0];
                                    const extdoc2 = response.EXTDOC;
                                    const urldoc2 = response.URLDOC;
                                    let file = '';
                                    switch (extdoc2) {
                                        case 'png':
                                        case 'jpg':
                                        case 'jpeg':
                                            file = `<img src="http://172.16.15.20${urldoc2}" style="width:100%;" alt="">`
                                            break;
                                        case 'pdf':
                                            file = `<embed src="http://172.16.15.20${urldoc2}" style="width: 100%; height: 85vh;">`
                                            break;
                                        default:
                                            file = `<h5>El formato del documento no tiene previsualización.</h5>`
                                            break;
                                    }
                                    let isActive = k == 0 ? 'active' : '';
                                    var optionText = Array.from(selectElement.options).find(option => option.value === response.TIPDOC);
                                    var textoSelect = "";
                                    if (optionText) {
                                        textoSelect = optionText.textContent || optionText.innerText;
                                    }
                                    let descrp2 = response.DESCRP;
                                    if (descrp2 == 'S-N') {
                                        descrp2 = '';
                                    }
                                    var depaUrl = "/API.LovablePHP/ZLO0016P/GetDepa/?coddep=" + response.CODDEP + "&secdep=" + response.CODSEC + "";
                                    var responseDepa = ajaxRequest(depaUrl);
                                    var depa = '';
                                    if (responseDepa.code == 200) {
                                        depa = responseDepa.data['SECDES'];
                                    }
                                    const depasec = response.CODDEP + " - " + response.CODSEC + " " + depa;
                                    var campos2 = {
                                        "cam0": response.CAM1,
                                        "cam1": response.CAM2,
                                        "cam2": response.CAM3,
                                    ...
                                    var urlCampos2 = "/API.LovablePHP/ZLO0015P/ListTiposFind/?tipo=" + response.TIPDOC;
                                    var responseCampos2 = ajaxRequest(urlCampos2);
                                    let inputs = '';
                                    if (responseCampos2.code == 200) {
                                        var camposDes2 = responseCampos2.data[0].CAMPOS.split("/");
                                        var cont2 = 0;
                                        var length2 = camposDes2.length;
                                        for (let i = 0; i < length2; i++) {
                                            var descripcion2 = "";
                                            if (camposDes2[i].toLowerCase() == "tienda" || camposDes2[i].toLowerCase() == "compañia") {
                                                var urlDes = "/API.LovablePHP/ZLO0001P/FindComarc/?compFiltro=" + campos2[
                                                    'cam' + i] + "";
                                                var responseDes = ajaxRequest(urlDes);
                                                descripcion2 = (responseDes.code == 200) ? responseDes.data[0]['COMDES'] : "";
                                            } else if (camposDes2[i].toLowerCase() == "proveedor") {
                                                var id = campos2['cam' + i].split("-");
                                                var tipo = id[0];
                                                var prov = id[1];
                                                var urlFind = "/API.LovablePHP/ZLO0015P/ProveedoresFind/?tipo=" + tipo +
                                                    "&proveedor=" + prov + "";
                                                var responseFind = ajaxRequest(urlFind);
                                                descripcion2 = (responseFind.code == 200) ? responseFind.data[0]['ARCNOM'] : "";
                                            } else if (camposDes2[i].toLowerCase() == "numero doc. fiscal") {

                                                if (campos2['cam' + i] == '99999999999999999999') {
                                                    campos2['cam' + i] = '';
                                                }
                                            }

                                            inputs += `<div class="col-6 col-lg-2 mt-2">
                                                <h6 class=" mt-1">` + camposDes2[i] + `:</h6></div>
                                                <div class="col-6 col-lg-4 mt-2">
                                                <label class="text-start form-control">` + campos2['cam' + i] + ` ` + descripcion2
                                                    .toUpperCase() + `</label>
                                                </div>`;
                                        }
                                    }
                                    carouselItems += `
                                        <div class="carousel-item ${isActive}">
                                            <div class="d-flex justify-content-center p-1">
                                                <div>
                                                <h5 class="mx-3">Orden: <span id="numord">${response.CAM4}</span></h5>
                                                </div>
                                                <div>
                                                <a
                                                    class="ms-4"
                                                    data-bs-toggle="collapse"
                                                    href="#collapseExample${k}"
                                                    role="button"
                                                    aria-expanded="false"
                                                    aria-controls="collapseExample${k}"
                                                >
                                                    Detalles <i class="fa-solid fa-circle-info"></i>
                                                </a>
                                                </div>
                                            </div>
                                            <div class="collapse border border-0" id="collapseExample${k}">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="">
                                                        ...
                                                        <div class="row mb-2">
                                                            ...
                                                            <div class="col-6 col-lg-4">
                                                            <labelclass= class="text-start form-control">
                                                                ${formatFecha(response.FECHA)}
                                                            </labelclass=>
                                                            </div>
                                                        ...
                                                        <div class="col-6 col-lg-4">
                                                            ...
                                                            >
                                                            ${formatFecha(response.FECGRA) + ' ' + formatTime(response.HORGRA)}
                                                            </label>
                                                        </div>
                                                        </div>
                                                    </div>
                    ...
                    $("#ordenesDiv").empty();
                    $("#ordenesDiv").append(`<div class="row">
                            <div class="col-12 p-3">
                                <div class="d-flex justify-content-center">
                                <div>
                                    <h5 class="mx-3">Orden: <span id="numord"></span></h5>
                                </div>
                                <div>
                                    <a
                                    class="ms-4"
                            ...
                            <div class="col-12">
                                <div class="collapse border border-0" id="collapseExample2">
                                <div class="card card-body">
                                    <div class="row">
                                    <div class="col-12">
                                        <div class="">
                                        <div class="">
                                            ...
                                            <div class="col-6 col-lg-4">
                                                <label class="text-start form-control" id="tipDoc2"></label>
                                            </div>
                                            <div class="col-6 col-lg-2">
                                                <h6 class="mt-1">Fecha de documento:</h6>
                                            </div>
                                            ...
                                            <div class="col-12 col-lg-4 mt-2">
                                                <span class="text-justify" id="descrpDoc2"></span>
                                            </div>
                                            <div class="col-12 col-lg-2 mt-2">
                                                <h6 class="mt-1">Departamento:</h6>
                                            </div>
                                        ...
                                        <div class="row" id="extraInfo2"></div>
                                        <div class="row">
                                            <div class="col-12">
                                            <hr />
                                            </div>
                                        ...
                                        <div class="row mt-2">
                                            ...
                                            <div class="col-6 col-lg-4">
                                            <label class="text-start form-control" id="usuaGra2"></label>
                                            </div>
                                            <div class="col-6 col-lg-2">
                                            <h6 class="mt-1">Fecha digitalización:</h6>
                                            </div>
                                            <div class="col-6 col-lg-4">
                                            <label class="text-start form-control" id="horaGra2"></label>
                                            </div>
                                        </div>
                                        </div>
                                    </div>
                            ...
                            <div class="p-0 m-0 mt-2" id="downloadFrame2"></div>
                            `);
                    setTimeout(() => {
                        var baseUrl = "/API.LovablePHP/ZLO0016P/ListAsync/";
                        var queryParams = [];
                        ...
                        if (cam0) queryParams.push("CAM1=" + cam0);
                        if (cam1) queryParams.push("CAM2=" + cam1);
                        if (cam4) queryParams.push("CAM4=" + cam4);
                        $("#numord").text(cam4);
                        var urlList = baseUrl + "?" + queryParams.join("&");
                        $("#downloadFrame2").empty();
                        fetch(urlList, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json'
                            ...
                            if (data.code == 200) {
                                ...
                                switch (extdoc2) {
                                    ...
                                    case 'pdf':
                                        ...
                                        let pdf = document.createElement('embed');
                                        pdf.src = `http://172.16.15.20${urldoc2}`;
                                        pdf.style.width = '100%';
                                        pdf.style.height = '85vh';
                                ...
                                $("#tipDoc2").text(textoSelect);
                                $("#fechaDoc2").text(formatFecha(response.FECHA));
                                let descrp2 = response.DESCRP;
                                if (descrp2 == 'S-N') {
                                    descrp2 = '';
                                }
                                $("#descrpDoc2").text(descrp2);
                                var depaUrl = "/API.LovablePHP/ZLO0016P/GetDepa/?coddep=" + response.CODDEP + "&secdep=" + response.CODSEC + "";
                                var responseDepa = ajaxRequest(depaUrl);
                                var depa = '';
                                if (responseDepa.code == 200) {
                                ...
                                $("#depaDoc2").text(response.CODDEP + " - " + response.CODSEC + " " + depa);
                                $("#usuaGra2").text(response.USUGRA);
                                $("#horaGra2").text(formatFecha(response.FECGRA) + ' ' + formatTime(response.HORGRA));
                                var campos2 = {
                                    "cam0": response.CAM1,
                                    "cam1": response.CAM2,
                                    "cam2": response.CAM3,
                                ...
                                if (responseCampos2.code == 200) {
                                    ...
                                    for (let i = 0; i < length2; i++) {
                                        ...
                                        inputs2.append(`<div class="col-6 col-lg-2 mt-2">
                                                <h6 class=" mt-1">` + camposDes2[i] + `:</h6></div>
                                                <div class="col-6 col-lg-4 mt-2">
                                                <label class="text-start form-control">` + campos2['cam' + i] + ` ` + descripcion2
                                                .toUpperCase() + `</label>
                ...
                Promise.all(fetchPromises).then(() => {
                    if (countOrdenes > 1) {
                        let div = `<div class="carousel carousel-dark slide">
                                        <button class="carousel-control-prev ms-5 ps-5" type="button" data-bs-target="#carouselExample" data-bs-slide="prev">
                                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                            <span class="visually-hidden">Previous</span>
                                        </button>
                                        <p class="text-center">Ordenes encontradas: ${countOrdenes}</p>
                                        <button class="carousel-control-next me-5 pe-5" type="button" data-bs-target="#carouselExample" data-bs-slide="next">
                                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                            <span class="visually-hidden">Next</span>
                                        </button>
                                      </div>
                                      <div id="carouselExample" class="carousel carousel-dark slide">
                                        <div class="carousel-indicators">${buttons}</div>
                                        <div class="carousel-inner">${carouselItems}</div>
                                      </div>`;
                        $("#ordenesDiv").empty();
                        $("#ordenesDiv").append(div);
                    }
                }).catch(error => {
                        ...
                        if (data[i]['ACCION'] == 'E') {
                            ...
                            if (data[i]['ACCION'] == 'A') {
                                ...
                                if (responseAuth.code == 200) {
                                    ...
                                    $("#AuthDoc").append(
                                        ` <button type="button" class="btn btn-success text-white mt-1 fw-bold" style="width:100%;"  onclick="authCard('` +
                                        nomcard +
                                        `','` + usuario + `','` + fecgra + `','` + horgra + `','` + extdoc + `','` +
                                        urldoc +
                                        `')">
    ...
    <div class="modal fade" id="docInfo2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Información del documento</h1>
                    <button type="button" class="btn-close" onclick="$('#docInfo2').modal('hide')"></button>
                </div>
                    ...
                    <div class="col-6">
                        <div class="row">
                            <div class="col-12 p-3">
                                <div class="d-flex justify-content-center">
                                    <div>
                                        <h5 class="mx-3">Factura: <span id="numfac"></span> </h5>
                                    </div>
                                    <div>
                                        <a class="ms-4" data-bs-toggle="collapse" href="#collapseExampleK" role="button"
                                            aria-expanded="false" aria-controls="collapseExampleK">
                                            Detalles <i class="fa-solid fa-circle-info"></i>
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div class="col-12">
                                <div class="collapse border border-0" id="collapseExampleK">
                                    <div class="card card-body">
                                        <div class="row">
                                            <div class="col-12">
                                                <div class="">
                                                    <div class="">
                                                        <div class="row mb-2">
                                                            ...
                                                            <div class="col-12 col-lg-4 mt-2">
                                                                <span class="text-justify " id="descrpDoc1"></span>
                                                            </div>
                                                            <div class="col-12 col-lg-2 mt-2">
                                                                <h6 class=" mt-1">Departamento:</h6>
                                                            </div>
                        ...
                        <div class="p-0 m-0 mt-2" id="downloadFrame1">
                        </div>
                    </div>
                    <div class="col-6" id="ordenesDiv">
