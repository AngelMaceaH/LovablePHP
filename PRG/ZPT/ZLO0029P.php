...
<body>
  ...
  ?>
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
                ...
                <li class="breadcrumb-item active"><span>ZLO0029P</span></li>
            </ol>
        </nav>
    </div>
    </header>
    <div id="body-div" class="body flex-grow-1">
        <div class="card mb-5">
            <div class="card-header">
                <h1 class="fs-4 mb-1 mt-2 text-center">Histórico meses de inventario por línea</h1>
            </div>
            <div class="card-body">
                                    ...
                                    <form>
                                        <div class="row mb-2">
                                            ...
                                            <div class="col-12 col-lg-3">
                                                <label class="mt-2">Línea</label>
                                                <select class="form-select mt-1 fw-bold" id="cbbLinea">
                                                </select>
                                            </div>
    ...
    window.addEventListener('DOMContentLoaded', (event) => {
       ...
       let urlTiendas = 'http://172.16.15.20/API.LovablePHP/ZLO0015P/ListTiendas/?user=' + usuario + '';
        ...
        if (responseTiendas.code == 200) {
            tiendasOptions += '<option class="fw-bold" value="0" disabled>Punto de Venta</option>';
        ...
        let groupTiendas=document.getElementById('groupTiendas');
        groupTiendas.innerHTML = tiendasOptions;
        const cbbLinea = document.getElementById('cbbLinea');
        let urlMarcas="http://172.16.15.20/API.LovablePHP/ZLO0029P/ListMarcas/";
        let responseMarcas = ajaxRequest(urlMarcas);
        let marcasOptions = '';
        if (responseMarcas.code == 200) {
            for (let i = 0; i < responseMarcas.data.length; i++) {
                marcasOptions += '<option value="' + responseMarcas.data[i].DESCO1 + '">' +
                    responseMarcas.data[i].DESDES + '</option>';
            }
        }
        cbbLinea.innerHTML = marcasOptions;
        ...
        let valLinea = cbbLinea.value;
        chargeTable(valAno, valAgrup,valLinea);
        chargeTable2(valAno2, valAgrup,valLinea);
        chargeTable3(valAno, valAgrup,valLinea);
        chargeTable4(valAno2, valAgrup,valLinea);

        cbbLinea.addEventListener('change', (event) => {
            let valAno = parseInt(cbbAno.value);
            let valAno2 = valAno - 1;
            let valAgrup = cbbAgrup.value;
            let valLinea = cbbLinea.value;
            ...
            chargeTable4(valAno2, valAgrup,valLinea);
            setTimeout(() => {
              chargeTable5(valAno, valAgrup,valLinea);
              chargeTable6(valAno2, valAgrup,valLinea);
                chargeGrafica();
                chargeGrafica2();
                chargeGrafica3();
            }, 700);
        ...
        setTimeout(() => {
            ...
            chart1 = Highcharts.chart('container', {
                ...
                title: {
                    text: cbbGrafica.options[cbbGrafica.selectedIndex].text + '<br>'+cbbLinea.options[cbbLinea.selectedIndex].text +' - '+ cbbAgrup
    ...
    function chargeTable(valAno, valAgrup, valLinea) {
        ...
        lineMIN12M1 = [];
        lineMIN06M1 = [];
        var urlList = "http://172.16.15.20/API.LovablePHP/ZLO0029P/List/?anopro=" + valAno + "&cia=" + valAgrup+"&linea="+valLinea;
    ...
    function chargeTable2(valAno, valAgrup, valLinea) {
        //AÑO 1
        linePRV12M2 = [];
        linePVR06M2 = [];
        lineMIN12M2 = [];
    ...
    //UNIDADES
    function chargeTable3(valAno, valAgrup, valLinea) {
        //AÑO 1
        lineUNICOM1 = [];
        lineUNIVEN1 = [];
        lineUNIEXI1 = [];
        var urlList = "http://172.16.15.20/API.LovablePHP/ZLO0029P/ListUnd/?anopro=" + valAno + "&cia=" + valAgrup+"&linea="+valLinea;
    ...
    function chargeTable4(valAno, valAgrup, valLinea) {
        //AÑO 1
        lineUNICOM2 = [];
        lineUNIVEN2 = [];
        lineUNIEXI2 = [];
