...
<body>
          ...
          <div class="col-12">
            ...
            <div class="demo">
              ...
              <div id="panel1" class="tablist__panel p-3 border border-0" aria-labelledby="tab1" aria-hidden="false"
                ...
                <div class="row">
                  <div class="col-12 col-lg-6">
                    <button type="button" id="exportExcel1" class="btn btn-success text-light fs-6 text-center mb-3"
                      style="width:150px;">
                ...
                <table id="tableFacturacion1" class="table stripe table-hover " style="width:100% ;">
                  <thead class="border-lg sticky-top bg-white">
                    <tr>
                      ...
                      <th colspan="14" class=" border border-dark align-middle text-center">
                        <span class="fs-4">FACTURACIÓN <span id="lblCia1"></span> AÑO <span id="lblano1"></span></span>
                      </th>
                    </tr>
                    <tr>
                      <th colspan="1"
                      ...
                      <th colspan="14" class=" border border-dark text-center">
                        <span class="fs-6">EXPRESADO EN <span id="lblmon1"></span></span>
                      </th>
                    </tr>
                    <tr>
                  ...
                  <tbody id="tableFacturacion1Detalle">
              ...
              <div id="panel2" class="tablist__panel is-hidden p-3 border border-0" aria-labelledby="tab2"
                ...
                <div class="row">
                  <div class="col-12 col-lg-6">
                    <button type="button" id="exportExcel2" class="btn btn-success text-light fs-6 text-center mb-3"
                      style="width:150px;">
                      <i class="fa-solid fa-file-excel me-1"></i><b>Enviar a Excel</b>
                    </button>
                  </div>
                ...
                <table id="tableFacturacion2" class="table stripe table-hover " style="width:100% ;">
                  <thead class="border-lg sticky-top bg-white">
                    <tr>
                      <th colspan="1" class="border border-0 bg-white"></th>
                      <th colspan="7" class=" border border-dark align-middle bgYellow text-center">
                        <span class="fs-4">LOVABLE <span id="lblCia2"></span> AÑO <span id="lblano2"></span></span>
                      </th>
                    ...
                    <tr>
                      ...
                      <th colspan="2"
                        ...
                        <span class="fs-6" id="saldoIni1"></span>
                      </th>
                      <th colspan="1" class="border border-0"></th>
                    </tr>
                    <tr>
                  ...
                  <tbody id="tableFacturacion2Detalle">
  ...
  <script>
    ...
    var table =null; var table2 =null;
    ...
    function chargeTable1(agrup, ano) {
      ...
      switch (agrup) {
        case '11':
        case '9':
          $("#lblmon1").text("LEMPIRAS");
          break;
        default:
          $("#lblmon1").text("DÓLARES");
          break;
      }
      let response1 = [];
      const url = `http://172.16.15.20/API.LovablePHP/ZLO0038P/ListFac1/?agrup=${agrup}&ano=${ano}`;
      fetch(url)
        .then(response => response.json())
        .then(data => {
          if (data.code == 200) {
            $("#lblCia1").text($("#cbbAgrup").find('option:selected').text());
            $("#lblano1").text($("#cbbAno").val());
            const responseData = data.data;
            response1 = responseData;
            const table1 = $("#tableFacturacion1Detalle");
            table1.empty();
    ...
    function chargeChart1(response1) {
      let seriesData = response1.map(item => {
        return {
          name: item.NOMCIA,
          data: [
            parseFloat(item.ENE),
      ...
      Highcharts.chart('container1', {
        ...
        title: {
          text: "FACTURACIÓN " + $("#cbbAgrup").find('option:selected').text() + " AÑO " + $("#cbbAno").val(),
          align: 'center',
          style: {
            color: '#FFFFFF'
          }
    ...
    function chargeTable2(agrup, ano) {
      $("#loaderData").removeClass('d-none');
      let response2 = [];
      const url = `http://172.16.15.20/API.LovablePHP/ZLO0038P/ListFac2/?agrup=${agrup}&ano=${ano}`;
      fetch(url)
        .then(response => response.json())
        .then(data => {
          if (data.code == 200) {
            $("#lblCia2").text($("#cbbAgrup").find('option:selected').text());
            $("#lblano2").text($("#cbbAno").val());
            const responseData = data.data;
            response2 = responseData;
            const table2 = $("#tableFacturacion2Detalle");
            table2.empty();
            $("#saldoIni1").text(parseFloat(responseData.SALDO).toLocaleString('es-419', {
            ...
            meses.forEach(mes => {
              ...
              compras.push(parseFloat(responseData[mes][1]));
              retenciones.push(parseFloat(responseData[mes][2]));
              notaDebito.push(parseFloat(responseData[mes][3]));
              abonoCuentas.push(parseFloat(responseData[mes][4]));
              notaCredito.push(parseFloat(responseData[mes][5]));
              saldoCuentas.push(parseFloat(responseData[mes][6]));
              table2.append(`<tr>
                            <td> <button class="btn btn-light" onclick="showHistorial(this,'row-${cont}')" ><i class="fa-solid fa-caret-right"></i></button> </td>
    ...
    function chargeChart2() {
      ...
      setTimeout(() => {
        chart2 = Highcharts.chart('container2', {
          ...
          title: {
            text: ($("#selectGrafica").find('option:selected').text() + " " + $("#cbbAgrup").find('option:selected').text() + " AÑO " + $("#cbbAno").val()).toUpperCase(),
            align: 'center',
            style: {
              color: '#FFFFFF'
            }
          ...
          series: [{
            name: $("cbbAgrup").find('option:selected').text(),
            data: arrayData,
            color: '#FF0000'
          }],
          credits: {
    ...
    function showHistorial(button, id) {
