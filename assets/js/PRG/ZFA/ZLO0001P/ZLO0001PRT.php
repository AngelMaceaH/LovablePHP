...
$(document).ready(function() {
    ...
    if (responseEnca.code == 200) {
        for (let i = 0; i < responseEnca.data.length; i++) {
            ...
            $("#lblCliente").text(responseEnca.data[i]['FACCI2'] + '' + responseEnca.data[i]['FACCO5'] + ' ' +
                responseEnca.data[i]['DESCLI']);
            $("#lblTipo").text(responseEnca.data[i]['FACTI2']);
            $("#lblVendedor").text("Vendedor: " + responseEnca.data[i]['FACNU4'] + ' ' + responseEnca.data[i][
                'MAENO3'
            ]);
    ...
    if (responseDeta.code == 200) {
        ...
        for (let i = 0; i < responseDeta.data.length; i++) {
            ...
            $('#tr' + i + '').append("<td class='text-start'><b>" + responseDeta.data[i]['FACCO7'] +
                "</b></td>");
            $('#tr' + i + '').append("<td class='text-center'><b>" + responseDeta.data[i]['FACCO8'] +
                "</b></td>");
            ...
            $('#tr' + i + '').append("<td class='text-end'><b>" + parseFloat(responseDeta.data[i]['FACPR5'])
        ...
        var decimales = unidades.toLocaleString('es-419', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        }).toString().substr((unidades.toString().indexOf(".")) + 1, 2) / 12;
        ...
        $("#myTableBody").append(`<tr>
                                      ...
                                      <td class='text-end'><b></b></td>
                                      <td class='text-end'><b></b></td>
                                      <td class='text-end table-active'><b>` + tBruto.toLocaleString('es-419', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        }) + `</b></td>
                                      </tr>
