...
function chargeTasks() {
  ...
  let urlTasks = `http://172.16.15.20/API.LovablePHP/ZLO0034P/GetTasks/?ano=${ano}&mes=${mes}&usuario=${usuario}&area=${area}&servi=${servi}&opc=${opcion}&descri=${user}`;
    ...
    .then((data) => {
      ...
      if (data.code == 200) {
        ...
        responseData.forEach((task) => {
          ...
          if (nivel == 1) {
            if (task.ESTADO == "A") {
              buttons = `<li><button class="dropdown-item  fw-bold" type="button" onclick="confirmDelete('${task.NUMERO}','E',0,0)">Eliminar <i class="fa-solid fa-trash text-danger"></i></button></li>
      ...
      if (data.code == 200) {
        ...
        switch (responseData[0].ESTADO) {
          ...
          case "B":
            ...
            break;
          case "F":
            let urlFichaTecnica2 = `http://172.16.15.20/API.LovablePHP/ZLO0034P/GetInstructionsF/?cia=${cia}&caso=${caso}`;
            fetch(urlFichaTecnica2)
              ...
              .then((data) => {
                if (data.code == 200) {
                              ...
                              <div class="col-2"></div>
                                <div class="col-8">
                                  <div class="input-group ">
                                    <input id="inputDescripFinal" maxlength="50" type="text" class="form-control rounded-start"
                  ...
                  footerCase.innerHTML = `<button type="button" class="btn btn-secondary rounded text-white fw-bold" data-bs-dismiss="modal">CANCELAR</button>
          ...
          case "C":
            ...
            let urlFichaTecnica = `http://172.16.15.20/API.LovablePHP/ZLO0034P/GetInstructionsF/?cia=${cia}&caso=${caso}`;
            fetch(urlFichaTecnica)
              .then((response) => response.json())
              .then((data) => {
                if (data.code == 200) {
    ...
    .then((data) => {
      if (data.code == 200) {
        ...
        if (responseData.EXTEN != "") {
          ...
          divFile = `<input id="imagenInputEditA" type="file" class="form-control rounded-start rounded-end m-0 w100">`;
        }
  ...
  fetch(urlHeader)
    ...
    .then((data) => {
      if (data.code == 200) {
        ...
        console.log(formatFecha(responseData[0].FECEN2.toString(), 1));
                ...
                .then((data) => {
                  if (data.code == 200) {
                    if ($(this).is(":checked")) {
                      ...
                      const urlAsig = `http://172.16.15.20/API.LovablePHP/ZLO0034P/AsignarColor/?caso=${numero}&nivel=${correl}&usuario=${user}`;
                      fetch(urlAsig)
                        .then((response) => response.json())
                        .then((data) => {
                          if (data.code == 200) {
...
function editColor(id, color) {
  ...
  saveButton.onclick = function () {
    ...
    const urlEdit = `http://172.16.15.20/API.LovablePHP/ZLO0034P/EditarColor/?nivel=${id}&descri=${newValue}&color=${color}&usuario=${user}`;
    fetch(urlEdit)
      .then((response) => response.json())
      .then((data) => {
        if (data.code == 200) {
...
function saveColor(form) {
  ...
  const url = `http://172.16.15.20/API.LovablePHP/ZLO0034P/CreateColor/?descri=${descripcion}&color=${color}&usuario=${user}`;
  fetch(url)
    .then((response) => response.json())
    .then((data) => {
      if (data.code == 200) {
