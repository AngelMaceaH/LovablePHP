      ...
      case "APAVXC":
        ...
        columnsConfig.push({
          ...
          render: function (data) {
            ...
            return cantidad != 0
              ? `<button onClick="cantidadesByStyle(this,1,${cantidad})" class="btn btn-light p-0 m-0 fs-14 px-1" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-title="Ver detalles de apartados">${cantidad}</button>`
              : " ";
          },
        });
        break;
      ...
      case "INVAPA":
        columnsConfig.push({
          ...
          render: function (data) {
            ...
            return cantidad != 0
              ? `<button onClick="cantidadesByStyle(this,0,${cantidad})" class="btn btn-light p-0 m-0 fs-14 px-1" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-title="Ver detalles de apartados">${cantidad}</button>`
              : " ";
          },
        });
        break;
...
function chargeTable(
  ...
  //INICIO
  url = `http://172.16.15.20/API.LovablePHP/ZLO0013PA/List/?marca=${marca}&plan=${plan}&estado=${estado}&btnor=${btnor}&inventarios=${inventarios}&clasificacion=${clasificacion}&orden=${orden}&filtro=${filtro}&repro=${repro}&formato=${formato}&searchVal=${searchVal}`;
  console.log(url)
...
function openModalVentas(estilo, data) {
  ...
  var urlResumen =
    "/API.LovablePHP/ZLO0013PA/ListResumen/?ano=" +
    anoActual +
    "&estilo=" +
    estilo +
    "";
...
function chargeDetalles(numano, estilo) {
  ...
  var urldeta =
    "/API.LovablePHP/ZLO0013PA/ListDeta/?marca=" +
    marcaStablished +
    "&plan=" +
    plan +
    "&estado=" +
...
function cantidadesByStyle(button, vend, total) {
  const row = button.closest("tr");
  const cells = row.querySelectorAll("td");
  ...
  fetch("/API.LovablePHP/ZLO0013PA/CantidadApar", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
